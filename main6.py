...
# --- RL core hyperparameters (DQN-style) ---
FRAME_STACK     = 4          # DQN-style 4-frame decision stack
HOLD_FRAMES     = 4          # action repeat (= frame-skip) decoupled from stack length
CNN_SIZE        = (84, 84)
...
REPLAY_SIZE       = 10000
LEARNING_STARTS   = 50_000   # warm-up (collect only) before any learning
TRAIN_FREQ        = 4        # train every N learner ticks (simple throttle)

HEALTH_LIMIT    = 98

# add these (or update if they exist)
LEARNING_RATE   = 5e-5          # smaller LR
REWARD_CLIP     = 1.0           # clip per-step rewards to [-1, 1]
FINAL_REWARD    = 1.0           # was 10.0
    ...
    def update_first_to_zero(self, pct1, pct2):
        ...
        if not self.round_winner_decided:
            ...
            if pct1 <= 2.0 and pct2 > 2.0 and self.first_to_zero_flag is None:
                ...
                log_round(f"⚠️ SIMULTANEOUS ZERO: P1={pct1:.1f}% P2={pct2:.1f}%, prev P1={self.prev_pct1:.1f}% P2={self.prev_pct2:.1f}%")
            ...
            # Lock "both at zero" (strict 0.0 here)
...
def consumer():
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            # No frame available - handle post-match and fallback actions using unified function
        ...
        if round_result and round_result[0] == "round_won":
            ...
            if prev_state is not None and prev_action is not None:
                ...
                zero_next_state  = np.zeros_like(prev_state, dtype=np.float32)
                zero_next_extras = np.zeros_like(prev_extra_feats, dtype=np.float32)
        ...
        if state == "waiting_for_match":
            ...
            if len(frame_stack) == FRAME_STACK and hold_counter <= 0:
                ...
                # E) ε selection on the new state WITH MASKING
...
def learner():
    ...
    while not stop_event.is_set():
        ...
        if buffer.len >= LEARNING_STARTS and (learn_tick % TRAIN_FREQ == 0):
